<template>
  <div class="art-chart_bar4">
    <div class="art-chart_bar4-canvas"></div>
  </div>
</template>
    ...
    methods: {
    	init () {
    		...
    		if(this.custom) { // 完全使用自定义配置
    			...
    			let lineColorArr = ["#0074ff", "#0bdba8"];
			    ...
			    if(dataAxis && data) {
							    ...
							    itemStyle: {
								    normal: {
									    color: {
										    ...
										    y2: 1,
										    colorStops: [{
											    offset: 0, color: 'rgba(97,234,200,1)' // 0% 处的颜色
										    }, {
											    offset: 1, color: 'rgba(11,219,168,1)' // 100% 处的颜色
										    }],
										    globalCoord: false // 缺省为 false
									    },
									    barBorderRadius: 2,
                    ...
                    str += '<li> <i style="background-color:' + lineColorArr[index] + '"> </i>' + item.seriesName + " "+ item.data + '%</li>'
							    ...
							    let html = '<div class="art-chart_bar4-tooltip"><h6>' + title + '</h6><ul>' + str + '</ul></div>';
							    return html
						    },
					    },
					    legend: {
