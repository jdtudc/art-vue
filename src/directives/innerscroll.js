...
export default {
		...
		setTimeout(() => {
			...
			if (pos == 'relative' || pos == 'absolute') {
				if (el.children && el.children.length === 1) {
					...
					let transStr = 'all 200ms linear';
					el.children[0].dataset.top = 0;
					let div = document.createElement('div'), span = document.createElement('span');
					div.className = 'art-inner-scroll-box';
					span.className = 'art-inner-scroll';
					setTransition(el.children[0], transStr);
					setTransition(span, transStr);
					setTransform(el.children[0], 'translate(0,0)');
					setTransform(span, 'translate(0,0)');
					div.appendChild(span);
					el.appendChild(div);

					let timer = setTimeout(() => {
						let mousewheel = "mousewheel", dis = 30, boxH = el.clientHeight,
							childH = el.children[0].clientHeight;
						if (isFF) { // Firefox浏览器
					...
					function setTransition(ele, transition) {
						ele.style.webkitTransition = ele.style.oTransition = ele.style.transition = transition;
					}
				} else {
					console.error("There can be only one container child element.")
				}
			} else {
				console.error("Container position should be relative or absolute.")
			}
		}, 0);
	},
	...
	unbind(el, binding, vnode) {
		...
		el.removeEventListener(mousewheel, function (event) {
		})
	}
}
